...
单文件、离线可用的数据工具箱，含 **策略回测实验室**：

| 页面 | 内容 |
|---|---|
| 概览 | 数据快照、最近 10 期、结构分布（实际占比 vs 理论值） |
| 走势图 | 三位数字走势 + 和值/跨度折线 |
| 遗漏统计 | 出现次数 / 频率 / 平均遗漏 / 最大遗漏 / 当前遗漏 |
| 频率分布 | 数字频率多窗口对比、和值/跨度/组态分布 |
| 选号器 | 直选复式 + 组选复式，算清覆盖概率与期望回报 |
| **策略回测** | 5 种策略严格样本外回测，与随机基线逐项对比 |

- 线上地址：<https://mmwei258.github.io/fucai3d-research/>
- 构建产物：[`docs/index.html`](./docs/index.html)
- 源码与构建说明：[`tool/README.md`](./tool/README.md)

**设计约束**：只做历史数据的统计呈现，不做任何号码预测；站内禁用
「必中 / 准确率 / 专家推荐 / 稳赚」等表述，不放任何联系方式与导流内容。
